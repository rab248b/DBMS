...
public class Execution {
	...
	public String selectQuery(LogicQueryNode LogicalQueryTree, Node rootNode){
		//preprocessSelect(LogicalQueryTree);		
		return result;
	}
...
//		
//		String new_relation_name= "select_"+tableName+"_"+Integer.toString(index++);
...
//		
//		if(condition.getRightOperandColumn()==0){// right operand is value
//			do {
//				if(j<memorySize-1){
...
//							if(Integer.parseInt(tempTupleR.getField(leftFieldName).toString())== Integer.parseInt(rightFieldValue.toString())){
...
//					  }
//					}
//					//insertIntoTable(new_relation_name, schema.getFieldNames(), field_values);
//				}
//			} while(j>0);		
...
//					//Apply Condition 
//					
//					if(schema.getFieldType(leftFieldName).toString().equals("STR20")&& schema.getFieldType(rightFieldName).toString().equals("STR20")){
//						if(tempTupleR.getField(leftFieldName).toString().equals(tempTupleR.getField(rightFieldName).toString()) ){
//							appendTupleToRelation(new_relation_reference, mem, mem.getMemorySize()-1, tempTupleR); 
//						  }
//					}else
//					if(schema.getFieldType(leftFieldName).toString().equals("INT")&& schema.getFieldType(rightFieldName).toString().equals("INT")){
//						if(condition.getOperator().equals("=")){
//								if(Integer.parseInt(tempTupleR.getField(leftFieldName).toString())== Integer.parseInt(tempTupleR.getField(rightFieldName).toString())){
//									appendTupleToRelation(new_relation_reference, mem, mem.getMemorySize()-1, tempTupleR);
//								  }
//						}else					
//						if(condition.getOperator().equals(">")){
...
//	}
	...
	public String applyCondition(String tableName, Condition condition) throws Exception{
		...
		if(condition.isLeftOperandList()){
			...
			case 0:			
				if(condition.getLeftOperandList()[1].equals("+")){
					leftFieldValue= Integer.parseInt(condition.getLeftOperandList()[0])+Integer.parseInt(condition.getLeftOperandList()[2]);
				}else
				if(condition.getLeftOperandList()[1].equals("-")){
					leftFieldValue= Integer.parseInt(condition.getLeftOperandList()[0])-Integer.parseInt(condition.getLeftOperandList()[2]);
				}else
      ...
      if(condition.isRightOperandList()){
			...
			case 0:			
				if(condition.getRightOperandList()[1].equals("+")){
					rightFieldValue= Integer.parseInt(condition.getRightOperandList()[0])+Integer.parseInt(condition.getRightOperandList()[1]);
				}else
				if(condition.getRightOperandList()[1].equals("-")){
					rightFieldValue= Integer.parseInt(condition.getRightOperandList()[0])-Integer.parseInt(condition.getRightOperandList()[1]);
				}else
      ...
      if(condition.getLeftOperandColumn()==0)
		...
		if(!condition.isLeftOperandList()&& !condition.isRightOperandList()){
			if(condition.getRightOperandColumn()==0){// right operand is value
				applyConditionWhenRightOperandValue( new_relation_reference, relation_reference,condition, leftFieldName, rightFieldValue);
			}else { //right operand is column
				applyConditionWhenRightOperandColumn(new_relation_reference, relation_reference,condition, leftFieldName, rightFieldName);
			}
		}
		...
		if(condition.isLeftOperandList()&& !condition.isRightOperandList()){
			if(condition.getRightOperandColumn()==0){// right operand is value
				...
				case 11:
					...
					applyConditionWhenRightOperandValueLeftListColumn( new_relation_reference, relation_reference,condition, leftFieldName1, leftFieldName2 , rightFieldValue);
					break;
				}
				...
				case 11:
					...
					//applyConditionWhenRightOperandValueLeftListColumn( new_relation_reference, relation_reference,condition, leftFieldName1, leftFieldName2 , rightFieldValue);
					applyConditionWhenRightOperandColumnLeftListColumn( new_relation_reference, relation_reference,condition, leftFieldName1, leftFieldName2 , rightFieldName);
					//applyConditionWhenRightOperandColumn(new_relation_reference, relation_reference,condition, leftFieldName, rightFieldName);
					break;
				}
	...
	public void applyConditionWhenRightOperandColumnLeftListColumn( Relation new_relation_reference, Relation relation_reference, Condition condition, String leftFieldName1, String leftFieldName2 , String rightFieldName){
		Schema schema= relation_reference.getSchema();
		ArrayList<FieldType> fieldTypes= new ArrayList<FieldType>();
		int numberOfBlocksR1= relation_reference.getNumOfBlocks();
		ArrayList<Tuple> tuplesOfR= new ArrayList<>();
		...
		do {
			...
			for (Tuple tempTupleR : tuplesOfR) {
				...
				if(schema.getFieldType(LeftField).toString().equals("INT")&& schema.getFieldType(rightFieldName).toString().equals("INT")){
					if(condition.getOperator().equals("=")){
	...
	public void applyConditionWhenRightOperandValueLeftListColumn(Relation new_relation_reference, Relation relation_reference, Condition condition, String leftFieldName1, String leftFieldName2 , Object rightFieldValue){
	...
	public  int getLeftFieldAdd(Condition condition, Tuple tempTupleR, String leftFieldName1, String  leftFieldName2){
			return	Integer.parseInt(tempTupleR.getField(leftFieldName1).toString())+Integer.parseInt(tempTupleR.getField(leftFieldName2).toString());
	}
	public  int getLeftFieldSubtract(Condition condition, Tuple tempTupleR, String leftFieldName1, String  leftFieldName2){
		return	Integer.parseInt(tempTupleR.getField(leftFieldName1).toString())-Integer.parseInt(tempTupleR.getField(leftFieldName2).toString());
}
	public  int getLeftFieldMultply(Condition condition, Tuple tempTupleR, String leftFieldName1, String  leftFieldName2){
		return	Integer.parseInt(tempTupleR.getField(leftFieldName1).toString())*Integer.parseInt(tempTupleR.getField(leftFieldName2).toString());
}
	public void applyConditionWhenRightOperandValue(Relation new_relation_reference, Relation  relation_reference, Condition condition, String leftFieldName, Object rightFieldValue ){
		Schema schema= relation_reference.getSchema();
		ArrayList<FieldType> fieldTypes= new ArrayList<FieldType>();
		int numberOfBlocksR1= relation_reference.getNumOfBlocks();
		ArrayList<Tuple> tuplesOfR= new ArrayList<>();
	...
	public void applyConditionWhenRightOperandColumn(Relation new_relation_reference, Relation  relation_reference, Condition condition, String leftFieldName, String rightFieldName){
		Schema schema= relation_reference.getSchema();
		ArrayList<FieldType> fieldTypes= new ArrayList<FieldType>();
		int numberOfBlocksR1= relation_reference.getNumOfBlocks();
		ArrayList<Tuple> tuplesOfR= new ArrayList<>();
	...
	/**
	 * 
	 * This method performs projection on th given table . It selects columns listed in columnNames. If it is * it projects all columns
	 * @param performProjection
	 * @param columnNames
	 * @return
	 */
	
	public String performProjection(String tableName, ArrayList<String> columnNames){
		clearMemory();
		Relation relation_reference= schema_manager.getRelation(tableName);
		ArrayList<FieldType> fieldTypes= new ArrayList<FieldType>();
	...
	public void  makeSortedSublist(String tableName, String columnName ,String sublist_relation_name, ArrayList<Tuple> tuplesOfR){
		...
		for (Tuple tempTupleR : tuplesOfR) {
			...
			if (schema.getFieldType(columnName).toString().equals("INT")) {
				...
				// sortStrings();
				System.out.println("No sorting for Strings");
			}
		}
	...
	public String performCrossProduct(String table1Name, String table2Name){
		...
		if(numberOfBlocksR1>memorySize-2 && numberOFBloksR2>memorySize-2){
			...
			do {
				if(j<memorySize-2){
					...
					tableR.getBlocks(k, 1, j); //get as many blocks of table R and place in memory, starting at index 1
					numberOfBlocks=j;
					j=0;
				}
				else{
		...
		tableR.getBlocks(0, 1, tableR.getNumOfBlocks()); //get all the blocks of table R and place in memory, starting at index 1
		...
		System.out.println("Total number of tuples in r "+ tableR.getNumOfTuples());
		System.out.println("Total number of tuples in s"+ tableS.getNumOfTuples());
		System.out.println("Total number of tuples in final table "+ new_relation_reference.getNumOfTuples());
	...
	public void clearMemory(){
		for(int k=0; k<mem.getMemorySize();k++){
			...
			block.clear();
			System.out.println(mem+"clear memory");
		}
	}
	...
	public String createTable(String relationName, ArrayList<String> field_names,
	 ArrayList<FieldType> field_types) {
		try {			
			Schema schema = new Schema(field_names, field_types);
			Relation relation_reference = schema_manager.createRelation(relationName, schema);
//			System.out.println("");
			System.out.println("Relation "+ relationName+ " created successfully.");
			result = relationName;
//			result = "Relation "+ relationName+ " created successfully.";
	...
	public String insertIntoTable(String relationName, ArrayList<String> field_names, ArrayList<Object> field_values){
		...
		Block block_reference = mem.getBlock(mem.getMemorySize()-1); // access to memory block 0
	  ...
	  public  String droptable(ArrayList<String> tableList){
		...
		for (String temp : tableList) {
			if (null == schema_manager.getRelation(temp)) {
				...
				System.out.print("Initially  the relation contains: " + "\n");
				System.out.print(relation_reference + "\n" + "\n");
				schema_manager.deleteRelation(temp);
				if(null == schema_manager.getRelation(temp))
				System.out.println("Deleted table" + temp);
	...
	//Get the list of all columns in the relation
	  public void getColumns(String relationName, List<String> columnList, List<FieldType> columnTypeList){
		  Relation relation_reference = schema_manager.getRelation(relationName);
			Schema schema = relation_reference.getSchema();
	  ...
	  public String performSimpleSorting(String tableName, String columnNameWithTable){
			...
			int k=0;
			int numberOfBlocks=0;
			String new_relation_name= "sort_"+Integer.toString(index++)+"_"+tableName;
			createTable(new_relation_name,schema.getFieldNames(),schema.getFieldTypes());	
			Relation new_relation_reference= schema_manager.getRelation(new_relation_name);
			...
			System.out.println("Final table contains sorted table \n   "+ new_relation_reference);
			return result = new_relation_name;
		}
	  
	  /**
	   * This method returns the ArrayList of Objects where first Object in the list is columnList and second Object is List of Tuples
	   * @param tableName
	   * @return
	   */
	  ...
	  public String performUnion( String table1, String table2){
			...
			do {
				...
				for (Tuple tempTupleS : tuplesOfS) {
					...
					/*if(!tuplesOfTable1.contains(tempTupleS)){						
			...
			System.out.println("Final table contains "+newRelationReference.getNumOfTuples()+" number of tuples");
